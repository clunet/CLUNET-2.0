...
ISR(CLUNET_INT_VECTOR)
{
	...
	switch (clunetReadingState)
	{
		...
		case CLUNET_READING_DATA:
			...
			/* Если линию прижало, значит была нулевая посылка - сбросим соответствующие биты */
			if (down)
				dataToRead[clunetReadingByteIndex] &= ~(255 >> clunetReadingBitIndex);

			/* Иначе если линию отжало, значит была единичная посылка - установим соответствующие биты */
			else
				dataToRead[clunetReadingByteIndex] |= (255 >> clunetReadingBitIndex);
